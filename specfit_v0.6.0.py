...
class App(qt.QWidget):
            ...
            if(not(self.xpos == None)):
                ...
                if len(data) > 0:
                    ...
                    if len(self.contfit[0]) > 0:
                        ...
                        zB = ((zcent - 0.01*zcent), (zcent + 0.01*zcent))#TODO:Is this too constraining?
    ...
    def Fitter(self,func,data,flux,err,wl,bounds,name = ''):
        ...
        mymc = mcmc.fit(func,wl,flux,err, 5000,*bounds) #was 1000
        hists = []
        ps = []
        #perc = []
        test = []
        ...
        for i in range(len(mymc[0])):
            ...
            if(np.max(hists[i][0]) >= 8*np.mean(hists[i][0])):#For distributions that are essentially "Noise" it seems best to simply take the mean value
                ...
                ps.append(np.percentile(mymc[0][i],50))
            '''
            plt.figure(i)
            plt.plot(hists[i][1][1:],hists[i][0])
            plt.hlines(np.mean(hists[i][0]),xmin=np.min(hists[i][1]),xmax=np.max(hists[i][1]),colors='r')
        ...
        if name == 'continuum':
            ...
            if name in self.pdfs.keys():
                name = self.cname + name #adding on extra name to differentiate continuum
    ...
    def onClick(self,event):
        ...
        if self.isMask:#Continuum bounding
            ...
            # of the right bound
            # TODO: Need to have handler for the case where no points are chosen
